...
def main(csv_path, output_dir, random_seed=None):
    ...
    for speaker, speaker_data in df.groupby('speaker'):
        ...
        df.loc[speaker_data.iloc[:train_end].index, 'block'] = 'B1'
        df.loc[speaker_data.iloc[train_end:val_end].index, 'block'] = 'B2'
        df.loc[speaker_data.iloc[val_end:].index, 'block'] = 'B3'
